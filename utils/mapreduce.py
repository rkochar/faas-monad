...
def mapreduce():
    ...
    lambda_mapper = m.create_lambda(mapper_lambda_name, "mapreduce/mapper", "mapper.lambda_handler", role=lambda_iam_role, template="mq_s3", mq_topic=mapper_queue, environment=mapper_environment | {"QUEUE_NAME": "mapper"}, ram=config["lambdaMemory"], timeout_seconds=150, is_timed=True, is_ram=True)
    ...
    lambda_reducer = m.create_lambda(reducer_lambda_name, "mapreduce/reducer", "reducer.lambda_handler", role=lambda_iam_role, template="mq_s3", mq_topic=reducer_queue, environment=reducer_environment, ram=config["lambdaMemory"], timeout_seconds=150, is_timed=True, is_ram=True)
